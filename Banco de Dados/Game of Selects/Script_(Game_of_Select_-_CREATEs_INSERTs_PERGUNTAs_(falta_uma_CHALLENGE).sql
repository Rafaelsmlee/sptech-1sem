...
/* 1.  O personagem principal de One Piece é o carismático Monkey D.Luffy que nasceu no ano de 2002, para começar o nosso desafio!
Selecione os piratas que nasceram em 2002 e que tenha uma recompensa maior de '930000000' belli 
*/
SELECT *
FROM pirata 
...
/* 2. Existem muitos personagens nos mares de One Piece, para explorar o mundo mágico de One Piece e conhecer os seus habitantes melhor selecione os piratas que nasceram no mês de julho e que termine com a letra "i" de forma crescente 
*/
...
ORDER BY nome ASC;
/*
 3. A espécie de aranha Rosamygale Grauvogeli é uma espécie  que existiu na pré história do nosso mundo(planeta Terra), o autor Eiichiro Oda trouxe alguns aspectos do nosso mundo para o universo de One Piece. A fruta do modelo Rosamygale Grauvogeli faz com que o usuário adquira poderes de animais pré-históricos, assim fazendo uma referência ao período que ela viveu no nosso mundo. 
 Selecione o nome da fruta, o poder, e o tipo que ela pertence somente onde a fruta for do modelo "Rosamygale Grauvogeli", ou as frutas que tem como terceira letra do nome 'u' e a antepenúltima letra do nome seja "a".
*/
select 
    nome,
    poder,
...
/* 4. Selecione o nome, recompensa (convertida de Belly para reais, supondo que 1 Belly seja igual a R$ 0,33), data de nascimento, e altura convertida em micrômetro (sabendo que está em centímetros, e que 1 centímetro é igual a 10000 micrômetros), apenas onde a recompensa é menor que 1001 Bellys ou para  os piratas que nasceram entre 1997 (que foi justamente no ano em que One Piece teve sua origem) e 2024 ou um dos mais velhos piratas que nasceu no ano de 1923 no mês de março e no dia 2.
...
/* 5.
As Akuma no Mi são frutas misteriosas e especiais espalhadas por todo o mundo, conhecidas por conceder aos seus consumidores poderes sobre-humanos permanentes, no entanto o usuário se torna permanentemente incapaz de nadar.
Como um todo, as Akuma no Mi abrangem centenas de poderes diferentes.
 Exiba o id da fruta, nome da fruta, seu poder, e seu tipo, apenas o top 5, onde os poderes das frutas podem ser capazes de: ou criar algo, ou controlar algo, ou se transformar 
...
-- 6. A relação entre os capitães e seus subordinados é um tema bastante explorado em One Piece, alguns temem, outros adimiram e tem até aqueles que desejam o posto de seu capitão. 
-- Selecione o nome, recompensa, a data de nascimento e a altura dos personagens que sejam subordinados.
select nome, recompensa, dtNascimento, altura
from pirata where fkCapitao >= 1;

-- 7. Os navios são as casas dos piratas, muitos passam anos e anos sem pisar em nenhuma terra firme e existem até navios que são transformados em restaurantes!
-- Selecione o nome e o navio principal dos bandos em que o navio principal tenha duas letras repetidas seguidas
...
-- 11. Muitos dos piratas que possuem uma recompensa maior, são porque representam mais perigo para o Governo Mundial, a maioria deles tem poderes bem peculiares e que normalmente veem das Fruta do Diabo. 
--  Ordene de uma forma decrescente as recompensas, nome dos piratas que comeram uma Fruta do Diabo.uais são as recompensas e os nomes dos piratas onde comeram uma Fruta do Diabo e tem uma recompensa, ordenados pela recompensa em ordem decrescente
...
-- 1.O bando do Chapéu de Palha é o bando do personagem principal Luff e obviamente recebe muitas atenções!
-- Exiba o nome do bando e do navio principal do Monkey D. Luffy, juntamente 
...
/* 3. Os Piratas Donquixote são um poderoso grupo, como seu capitão é um famoso pirata, um ex-membro dos Shichibukai com uma recompensa de 340.000.000
 Qual a soma das recompensas do bando do Donquixote e seu Navio Principal
 */
...
-- 4. Lavoisier há muito tempo atrás já diz que 'Na natureza nada se cria, nada se perde, tudo se transforma'. Mostre todos os piratas que tem o poder de transformar o usuário ou outras massas
...
-- Utilize a seguinte taxa de conversão 100 beli são aproximadamente  R$3,30 arredondando o valor final para 2 casas decimais. 

select p.nome,
	   round(p.recompensa * 0.033, 2) as recompensa_em_reais, 
	   altura
from pirata p
...
order by p.idPirata;
/* 2 Selecione o nome dos bandos (nomear coluna para 'Nome Bando'), nome dos piratas (nomear coluna para 'Nome pirata'), e a recompensa de cada um onde a recompensa não seja nula ordenado pelo bando pirata na ordem padrão.
 Adicione também uma coluna (nomeada como "recompensa_formatada") que exiba a recompensa de maneira formatada em Reais (BRL)
 (DICA: Quando a recompensa é em bilhão ela tem o comprimento de 10. As recompensas tem diferentes comprimentos, dentre elas estão as de: 10, 9, 8, 4 e 3)
 Exemplo: 3000000000 / Modo formatado: R$ 3.000.000.000
 Adicione uma coluna (nomeada como "fruta_formatada"), onde caso o pirata não tenha comido, exiba o seguinte texto: 'NÃO COMEU FRUTA!', caso contrário exiba o nome da fruta normalmente.
 E por fim, exiba a descrição do objetivo do pirata (apenas de quem tiver algum objetivo).
*/

select
bp.nome as 'Nome Bando',
p.nome as 'Nome pirata',
recompensa,
case
when length(recompensa) = 10 then concat('R$ ', substring(recompensa, 1, 1), '.', substring(recompensa, 2, 3), '.', substring(recompensa, 5, 3), '.', substring(recompensa, 8, 3))
when length(recompensa) = 9 then concat('R$ ', substring(recompensa, 1, 3), '.', substring(recompensa, 4, 3), '.', substring(recompensa, 7, 3))
when length(recompensa) = 8 then concat('R$ ', substring(recompensa, 1, 2), '.', substring(recompensa, 3, 3), '.', substring(recompensa, 6, 3))
when length(recompensa) = 4 then concat('R$ ', substring(recompensa, 1, 1), '.', substring(recompensa, 2, 3))
when length(recompensa) = 3 then concat('R$ ', substring(recompensa, 1, 3))
end as recompensa_formatada,
case
WHEN fruta.nome is null then 'NÃO COMEU FRUTA!'
ELSE fruta.nome
end as fruta_formatada,
ob.descricao
from pirata as p
inner join bandopirata as bp
on p.fkbandopirata = bp.idBando

left join frutadiabo as fruta
on p.fkFruta = fruta.idFruta
inner join pirataobjetivo as po
on po.fkpirata = p.idPirata

inner join objetivo as ob
on po.fkObjetivo = ob.idObjetivo
where recompensa is not null
order by p.fkbandopirata;
