...
INSERT INTO pedido_item (fkpedido, fkproduto, quantidade, valor_unitario) VALUES
 ...
 pedidoItem.valor_unitario as 'Valor unitário',
  round(pedidoItem.quantidade  *  pedidoItem.valor_unitario,2) as 'Valor Total'
 
 from cliente 
 inner join pedido on  pedido.fkcliente = cliente.id
 inner join pedido_item as pedidoItem on pedidoItem.fkpedido = pedido.id
  join produto on produto.id = pedidoItem.fkproduto;
 ...
 from cliente 
 left join pedido on  pedido.fkcliente = cliente.id
 ...
 3- Listar produtos que não foram vendidos, ordenados por valor unitário do maior para o menor*/
  ...
  prod.preco_unitario
  from produto prod
  left join pedido_item as ite on  ite.fkproduto = prod.id
  where ite.fkpedido is null
  order by prod.preco_unitario desc;
  
  select prod.nome
  from produto prod
  left join pedido_item ite on ite.fkproduto = prod.id
 ...
 /* 4 Listar todos os clientes, mesmo que não tenham realizado compras e para os que compraram informar os produtos, com valor total do item*/
   ...
   ped.id pedido,
   prod.nome produto,
     round(ite.quantidade  *  ite.valor_unitario,2) as 'Valor Total'
  from cliente cli
  left join pedido as ped on ped.fkcliente = cli.id
  left join pedido_item as ite on ite.fkpedido = ped.id
  left join produto as prod on prod.id = ite.fkproduto;
