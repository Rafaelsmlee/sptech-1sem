...
(
    ...
    key fkestado(fkestado), -- indice para localizar mais facil
...
(
	...
	unique key sigla(sigla),
    key fkcidade(fkcidade) -- indice para localizar mais facil
...
DELIMITER $$
CREATE FUNCTION dados_db.fn_estado(fCodigo VARCHAR(20))   -- cria uma função para passar um fCodigo como VARCHAR
RETURNS int -- retorna um numero interno
DETERMINISTIC
BEGIN
	DECLARE vId int;
...
select dados_db.fn_estado('SP');
select * from dados_db.estado;


-- criação de view para consultar ID da Cidade atraves Estado da Sigla do Estado e Nome da Cidade
DELIMITER $$
CREATE FUNCTION dados_db.fn_cidade(fUF VARCHAR(02), fCidade varchar(100))  -- criou uma função para pegar o nome de uma tabela para outra
RETURNS int
DETERMINISTIC
BEGIN
	DECLARE vId int;
...
select * from dados_db.cidade;
select * from dados_db.estado;

-- 1. Consultar dados de cidade e estado exibindo a o Id da cidade, sigla do estado e o nome da cidade

select cd.id as 'Cidade ID',  est.id as 'Estado ID',  cd.nome as 'Nome da cidade'
from  dados_db.cidade as cd
join  dados_db.estado as est on cd.fkestado = est.id;

-- 2. Consultar dados de cidade, estado e aeroporto exibindo a sigla do estado, sigla do aeroporto
select est.sigla as 'Sigla do Estado' ,
 ae.sigla as 'Sigla do aeroporto da cidade'
from dados_db.aeroporto as ae
join dados_db.cidade as cid on ae.fkcidade = cid.id 
join  dados_db.estado as est on cid.fkestado = est.id ;

-- 3. Crie uma view de nome vw_cidade, com o comando do exercício no. 1

create view dados_db.vw_cidade as select   cid.id as 'Cidade ID',  est.id as 'Estado_ID',  cid.nome as 'Nome da cidade'
from  dados_db.cidade as cid
join  dados_db.estado as est on cid.fkestado = est.id;

select * from dados_db.vw_cidade ;

-- 4. Crie uma view de nome vw_aeroporto, com o comando do exercico no. 2
create view dados_db_vw_aeroporto as select est.sigla as 'Sigla_do_Estado' ,
 ae.sigla as 'Sigla do aeroporto da cidade'
from dados_db.aeroporto as ae
join dados_db.cidade as cid on ae.fkcidade = cid.id 
join  dados_db.estado as est on cid.fkestado = est.id ;

select * from dados_db_vw_aeroporto;

-- 5. Consultar as cidades do estado de São Paulo utilizando a view
select  *
from dados_db.vw_cidade
where Estado_ID = 25;

-- 6. Consultar os aeroportos do estado de Pernambuco utilizando a View
select  *
from dados_db_vw_aeroporto
where Sigla_do_Estado = 'PE';

-- 7 Altere a view de aeroporto, inclua o nome do aeroporto e execute a consulta filtrando pelos aeroportos que tenham a  primeira letra da sigla do aeroporto igual a “C”

alter view dados_db_vw_aeroporto as select nome as 'Nome_aeroporto'
from aeroporto;

select *
from dados_db_vw_aeroporto
where nome_aeroporto like 'S%';

select *
from dados_db_vw_aeroporto
where 'Sigla do aeroporto da cidade' like '%C'; -- Murilo vai quebrar o meu PC por ter usado '' no where

-- 8. Apague a view de cidades
drop view dados_db.vw_cidade ;